...
const TABS = [
  {
    ...
    body: "Pick a name. Pick a template. Press publish.",
    image: "/assets/10-mins-cover.png",
    cta: null,
    icon: (
  ...
  },
  {
    ...
    image: "/assets/lowcost-cover.png",
  ...
  },
  {
    ...
    cta: { label: "Apply for DGC →", href: "https://partners.onenetservers.net/" },
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export function FeatureTabs() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    ...
    const index = Math.min(TABS.length - 1, Math.floor(clamped / STEP));
    setActive(index);
  });

  function tabFill(i: number) {
    return Math.min(1, Math.max(0, (scrollProgress - i * STEP) / STEP));
  }

  return (
    <section className="ftabs-section" aria-label="What we offer">
      <div ref={trackRef} className="ftabs-track">
        <div className="ftabs-sticky">
          <div className="shell ftabs-inner">
            ...
            {/* ── Right: tab list ── */}
            <div className="ftabs-right">
              <h2 className="ftabs-heading">From idea to your website, sorted. Your first year, free.</h2>
            <div className="ftabs-list" role="tablist" aria-label="Features">
              {TABS.map((tab, i) => {
                const isActive = active === i;
                const fill = tabFill(i);
                return (
                  <div
                    key={tab.key}
                  ...
                  >
                    ...
                    {/* scroll progress bar */}
                    <div className="ftab__progress" aria-hidden="true">
                      <div
                        className="ftab__progress-fill"
